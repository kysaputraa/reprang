...
<body <?php if ($isi == 'print'){ echo "onload='window.print()'"; } ?> >
	...
	<table class="isi" border="1" width="100%">
		...
		<?php 
			$urusan = $this->db->join('bidang', 'bidang.id_urusan = urusan.id_urusan');
			$urusan = $this->db->join('program', 'program.id_bidang = bidang.id_bidang');
			$urusan = $this->db->join('kegiatan', 'kegiatan.id_program = program.id_program');
			$urusan = $this->db->where('kegiatan.id_pptk', $this->session->userdata('id'));
			...
			foreach ($urusan as $row) {
				...
				$bidang = $this->db->join('program', 'program.id_bidang = bidang.id_bidang');
				$bidang = $this->db->join('kegiatan', 'kegiatan.id_program = program.id_program');
				$bidang = $this->db->where('kegiatan.id_pptk', $this->session->userdata('id'));
				...
				foreach ($bidang as $rowbidang) {
					...
					$program = $this->db->join('kegiatan', 'kegiatan.id_program = program.id_program');
					$program = $this->db->where('kegiatan.id_pptk', $this->session->userdata('id'));
